...
async function saveAllCurrencies() {
  return await Promise.all(currencies.map(currency => new Currency(currency).save()))
}

async function saveAllOperations() {
  return await Promise.all(operations.map(operation => new Operations(operation).save()))
}
